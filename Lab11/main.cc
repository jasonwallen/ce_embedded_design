...
{ 
	...
	if(RegisterRead(ptr, gpio_sw1_offset))
	{
		...
		RegisterWrite(ptr, 0, switch_num);
	}
	...
	while(i < 5){
		...
		else if (RegisterRead(ptr, gpio_pbtnr_offset))
		{
			moves[i] = 2;
			i++;
		}
		else if (RegisterRead(ptr, gpio_pbtnu_offset))
		{
			moves[i] = 3;
			i++;
		}
		else if (RegisterRead(ptr, gpio_pbtnd_offset))
		{
			moves[i] = 4;
			i++;
		}
		else if(RegisterRead(ptr, gpio_pbtnc_offset))
		{
			for(int k = i; k < 5; k++)
				moves[k] = 0;
			break;
